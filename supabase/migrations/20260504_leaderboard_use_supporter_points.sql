-- Recreate the leaderboard RPC to source support points directly from
-- profiles.supporter_points rather than summing bmac_contributions.points_awarded.
--
-- This is the authoritative column updated by all admin flows (direct point edits,
-- BMAC donation recording, etc.), so using it ensures the leaderboard always
-- reflects what the admin has set.
...
as $$
    ...
    supporter_points_totals as (
        -- Support points read directly from profiles.supporter_points (authoritative)
        select
    ...
    select
        ...
        case
            ...
            then coalesce(sp.bmc_points, 0)::bigint
            else 0::bigint
        end                                                              as bmc_points,
        (
    ...
    left join supporter_points_totals sp
        on  coalesce(it.entity_type, cc.entity_type) = 'user'
        and sp.entity_id = coalesce(it.entity_id, cc.entity_id);
$$;
